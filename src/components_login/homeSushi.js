import { fetchShop3, fetchComments3 } from "../services/cartService";
...
import HeaderSushi from "./Headersushi";
const HomeSushiLogin = () => {
  const {
    state: { products },
  ...
  useEffect(() => {
    const fetchShopDetails = async () => {
      const data = await fetchShop3();
      setShop(data.DT[0]);  // Assuming data.DT is an array of shop details
    };

    const fetchShopComments3 = async () => {
      const data = await fetchComments3(3); // Assuming shop ID is 1
      setComments(data.DT);
    };
